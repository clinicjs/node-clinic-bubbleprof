...
class CanvasNodeSection {
  ...
  initializeFromData () {
    ...
    if (this.shapeClass === 'CanvasBubble') {
      this.d3InnerCircle = this.d3NodeGroup.append('circle')
        .classed('inner-circle', true)
    }

    const canvasNodeElementClasses = {
      CanvasLine: CanvasLine,
      CanvasBubble: CanvasBubble
    }

    const CanvasNodeElementClass = canvasNodeElementClasses[this.shapeClass]

    this.byParty = new CanvasNodeElementClass(this, this.d3NodeGroup, 'party')
      .setData(this.layoutNode)
      .initializeFromData()

    this.byType = new CanvasNodeElementClass(this, this.d3NodeGroup, 'typeCategory')
      .setData(this.layoutNode)
      .initializeFromData()
  }
  ...
  draw () {
    ...
    if (this.d3InnerCircle) {
      // draw the circle with canvas
      this.canvasContext.beginPath()
      this.canvasContext.fillStyle = '#ffff00' // TODO: styling - garish colours for now - to test
      this.canvasContext.arc(
        this.parentContent.circleCentre.x,
        this.parentContent.circleCentre.y,
        Math.max(this.parentContent.getRadius() - this.ui.settings.lineWidth * 4, 0),
        0,
        2 * Math.PI,
        true
      )
...
class CanvasNodeElement {
  ...
  animate (svgNodeAnimations, isExpanding) {
    ...
    const contractedCanvasLineElement = contractedSvgNode.asyncBetweenLines[dataTypeKey]
    ...
    if (this.constructor.name === 'CanvasBubble') {
    ...
    elementProperties.contractedOrigin = elementProperties.nodeWasBetweenInParent ? cloneXY(contractedCanvasLineElement.originPoint) : defaultArcOrigin()
  ...
  animateSegment (segmentDatum, d3LineSegment, resolveSegment, elementProperties) {
    ...
    const segmentDecimal = this.constructor.name === 'CanvasBubble' ? segmentDatum.data[1] : segmentDatum[1]
    const nodeTime = this.svgNode.layoutNode.node[(this.constructor.name === 'CanvasBubble' ? 'getWithinTime' : 'getBetweenTime')]()
    ...
    if (!nodeWasBetweenInParent && this.constructor.name === 'CanvasLine') {
...
class CanvasLine extends CanvasNodeElement {
  initializeFromData () {
  ...
  draw () {
    ...
    this.d3Shapes.each((segmentDatum, index, nodes) => {
      ...
      this.canvasContext.lineWidth = 3.5
      const canvaspath = new window.Path2D(segmentPath)
      this.canvasContext.stroke(canvaspath)
    })
  }
}

class CanvasBubble extends CanvasNodeElement {
  setData (layoutNode) {
...
module.exports = CanvasNodeSection

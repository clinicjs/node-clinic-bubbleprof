/**
 * converting from svg implementation to canvas rendered implementation
 */

'use strict'

const LineCoordinates = require('../layout/line-coordinates.js')
const CanvasNodeSection = require('./canvas-node-section.js')
...
class CanvasNode {
  constructor (parentContent) {
    ...
    // properties as data values rather than attributes or classes on sub-Dom nodes
    // to get make this less dependent on SVG rendering and allow for declarative view state
    // that can be rendered in a single draw function
    // i.e. we are moving state away from the DOM and into this object
    ...
    this.asyncBetweenLines = new CanvasNodeSection(this, {
      dataPosition: 'between',
      shapeClass: 'CanvasLine'
    })
    
    this.syncBubbles = new CanvasNodeSection(this, {
      dataPosition: 'within',
      shapeClass: 'CanvasBubble'
    })
  }
  ...
  /**
   * draws the arrow shape that links to the connected node cluster
   */
  drawShortcut () {
  ...
  /**
   * draw the name of this node cluster
   */
  drawNameLabel () {
    ...
    if (!this.layoutNode.children.length) {
      ...
      if (textAfterTrim !== labelPlusTime) {
        ...
        if (textAfterTrimToEdge.length > textAfterTrim.length) {
          ...
          this.canvasContext.rotate(this.labelDegrees)
          this.canvasContext.setTransform(1, 1, 1, 1, x2, y2)
    ...
    if (this.drawType === 'labelOnLine') {
      ...
      this.canvasContext.rotate(this.labelDegrees)
      this.canvasContext.setTransform(1, 1, 1, 1, toMidwayPoint.x2, toMidwayPoint.y2)
    ...
    this.canvasContext.fillText(textAfterTrim, this.circleCentre.x, this.circleCentre.y)
  }


  /**
   * draw the time label inside the circle
   */
  drawTimeLabel () {
    if (this.drawType !== 'labelInCircle') {
      this.d3TimeLabel.classed('hidden', true)
...
/**
 * draw the outline to the entire cluster shape
 */
  drawOuterPath () {
    let outerPath = ''
    ...
    let p = new window.Path2D(outerPath)
    this.canvasContext.stroke(p)
